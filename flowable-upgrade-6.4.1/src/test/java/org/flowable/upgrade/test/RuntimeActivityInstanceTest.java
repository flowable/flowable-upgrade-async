...
public class RuntimeActivityInstanceTest extends UpgradeTestCase {
    ...
    public void testCompleteTask() {
        ...
        waitForHistoryJobExecutorToProcessAllJobsForTenant("simpleTaskProcessTenant");
        ...
        assertHistoricActivityInstanceIsSame(activityInstance);

        ActivityInstance flowActivityInstance = runtimeService.createActivityInstanceQuery().activityId("flow3").activityTenantId("simpleTaskProcessTenant").singleResult();
        assertThat(flowActivityInstance, notNullValue());
        assertThat(activityInstance.getEndTime(), notNullValue());
        assertHistoricActivityInstanceIsSame(flowActivityInstance);
    }

    @Test
    public void testChangeAssigneeBeforeHistoricActivityCreated() {
        Task task = getTaskFromGeneratedData("changeAssigneeBeforeHistoricActivityCreated");

        taskService.unclaim(task.getId());
        taskService.claim(task.getId(), "newAssignee");
        waitForHistoryJobExecutorToProcessAllJobsForTenant("changeAssigneeBeforeHistoricActivityCreatedTenant");
        ...
        assertThat(activityInstance, nullValue());
    }
    ...
    public void testChangeAssignee() {
        ...
        taskService.unclaim(task.getId());
        taskService.claim(task.getId(), "newAssignee");
        waitForHistoryJobExecutorToProcessAllJobsForTenant("changeAssigneeTenant");
    ...
    public void testChangeOwner() {
        ...
        taskService.complete(task.getId());
        waitForHistoryJobExecutorToProcessAllJobsForTenant("changeOwnerTenant");
    ...
    public void testCallSimpleSubProcess() {
        ...
        taskService.complete(task.getId());
        waitForHistoryJobExecutorToProcessAllJobsForTenant("callSimpleSubProcessTenant");

        ActivityInstance activityInstance = runtimeService.createActivityInstanceQuery().processInstanceId(task.getProcessInstanceId()).activityId("theReceiveTask").singleResult();
        assertThat(runtimeService.createActivityInstanceQuery().processInstanceId(task.getProcessInstanceId()).activityId("subProcessTask").count(), is(0l));
        assertThat(activityInstance, notNullValue());
    ...
    protected Task getTaskFromGeneratedData(String processBusinessKey) {
        ...
        assertThat(managementService.createHistoryJobQuery().count(), greaterThan(0l));
        assertThat(runtimeService.createActivityInstanceQuery().processInstanceId(processInstance.getId()).count(), is(0l));
        return task;
    }

    protected void assertHistoricActivityInstanceIsSame(ActivityInstance activityInstance) {
    ...
    private void waitForHistoryJobExecutorToProcessAllJobsForTenant(String tenantId) {
        List<HistoryJob> historyJobs = managementService.createHistoryJobQuery().jobTenantId(tenantId).list();
